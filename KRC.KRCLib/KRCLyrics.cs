...
public class KRCLyrics
{
    ...
    public string SaveToAss()
    {
        ...
        foreach (var line in Lines)
        {
            ...
            sb.AppendLine(string.Format("Dialogue: 0,{0},{1},Default,,0,0,0,,{2}",
                FormatSubtitleTime(lineStart), FormatSubtitleTime(lineEnd), textPart));
        }
    ...
    /// 转换为SSA字幕格式字符串
    /// </summary>
    /// <returns>SSA字幕内容</returns>
    public string SaveToSsa()
    {
        var sb = new StringBuilder();
        ...
        // V4 Styles (SSA uses decimal BGR colours, no ScaleX/ScaleY/Spacing/Angle/Underline/StrikeOut)
        sb.AppendLine("[V4 Styles]");
        sb.AppendLine("Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, TertiaryColour, BackColour, Bold, Italic, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, AlphaLevel, Encoding");
        sb.AppendLine("Style: Default,Arial,48,16777215,65535,65535,-2147483640,0,0,1,2,1,2,10,10,10,0,1");
        sb.AppendLine();
        ...
        foreach (var line in Lines)
        {
            ...
            sb.AppendLine(string.Format("Dialogue: Marked=0,{0},{1},Default,,0000,0000,0000,,{2}",
                FormatSubtitleTime(lineStart), FormatSubtitleTime(lineEnd), textPart));
        }
    ...
    /// </summary>
    /// <param name="outputFilePath">输出文件路径</param>
    public void SaveToSsaFile(string outputFilePath)
    {
        File.WriteAllText(outputFilePath, SaveToSsa(), Encoding.UTF8);
    }

    /// <summary>
    /// 格式化时间为字幕时间格式 h:mm:ss.cc
    /// </summary>
    /// <param name="time">时间</param>
    /// <returns>格式化后的时间字符串</returns>
    private static string FormatSubtitleTime(TimeSpan time)
    {
